 ...
 *
 * 传输上下文，包含了用于创建传输服务端 TransportServer 和 传输客户端工厂 TransportClientFactory的上下文信息
 *  并支持使用 TransportClientHandler设置Netty提供的SocketClient的Pipeline的实现
 *  */
public class TransportContext {
  private static final Logger logger = LoggerFactory.getLogger(TransportContext.class);
  /** 传输上下文的配置信息 */
  private final TransportConf conf;
  /** 对调用传输客户端 TransportClient的sendRPC方法发送的消息进行处理的程序 */
   ...
   *
   * MessageEncoder: 在将消息放入管道前对消息进行编码，防止管道另一端读取时丢包和解析错误
   ...
   * to create a Client.
   *
   * 创建 TransportClientFactory
   * 参数 TransportClientBootstrap为列表，或者空列表
   */
  ...
  public TransportChannelHandler initializePipeline(
    ...
    try {
      // 调用 createChannelHandler 创建 TransportChannelHandler
      TransportChannelHandler channelHandler = createChannelHandler(channel, channelRpcHandler);
      // 根据Netty的API行为，通过addLast方法在注册多个Handler时，
      // ChannelInboundHandler 按照注册的先后顺序执行
      // ChannelInboundHandler 按照注册的先后顺序的逆序
      channel.pipeline()
        .addLast("encoder", ENCODER)  // 派生自 Netty的ChannelOutboundHandler接口
        .addLast(TransportFrameDecoder.HANDLER_NAME, NettyUtils.createFrameDecoder())
        .addLast("decoder", DECODER)  // 派生自 Netty的ChannelInboundHandler接口
  ...
  private TransportChannelHandler createChannelHandler(Channel channel, RpcHandler rpcHandler) {
    ...
    // TransportClient的创建只使用了 TransportResponseHandler
