...
public class TransportRequestHandler extends MessageHandler<RequestMessage> {
  ...
  public TransportRequestHandler(
    ...
    this.streamManager = rpcHandler.getStreamManager();  // streamManager来自 rpcHandler的调用
  ...
  public void handle(RequestMessage request) {
    // 处理 5种 RequestMessage的过程，org.apache.spark.network.protocol中
  ...
  private void processFetchRequest(final ChunkFetchRequest req) {
    ...
    ManagedBuffer buf;
    try {
      // 调用 checkAuthorization 方法，校验客户端是否有权限从给定的流中读取
      streamManager.checkAuthorization(reverseClient, req.streamChunkId.streamId);
      // 调用 getChunk，获取单个的块，块被封装为ManagedBuffer，单个流只能与单个的TCP相关联
    ...
    // 将ManagedBuffer和流的ID封装之后返回客户端  respond --->  writeAndFlush
