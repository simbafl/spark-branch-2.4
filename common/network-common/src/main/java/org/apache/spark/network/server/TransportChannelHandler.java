 ...
 *
 * 代理由 TransportChannelHandler 处理的请求和由 TransportResponseHandler 处理的响应，并加入传输层的处理
 */
public class TransportChannelHandler extends ChannelInboundHandlerAdapter {
  ...
  public void channelRead(ChannelHandlerContext ctx, Object request) throws Exception {
    // TransportChannelHandler 读取的request是 RequestMessage时，将request交由 requestHandler 处理
    if (request instanceof RequestMessage) {
      requestHandler.handle((RequestMessage) request);
      // 读取的request是 ResponseMessage时，将request交由 responseHandler 处理
