...
private[rpc] class NettyRpcEnvFactory extends RpcEnvFactory with Logging {
  ...
  def create(config: RpcEnvConfig): RpcEnv = {
    ...
    // 创建javaSerializerInstance用于RPC传输对象的序列化，在多线程下是安全的
    // 如果考虑支持KryoSerializer，那必须在ThreadLocal下存储SerializerInstance
    val javaSerializerInstance =
      new JavaSerializer(sparkConf).newInstance().asInstanceOf[JavaSerializerInstance];
    // 创建NettyRpcEnv其实就是对内部各个子组件实例化过程
    val nettyEnv = new NettyRpcEnv(sparkConf, javaSerializerInstance, config.advertiseAddress,
        config.securityManager, config.numUsableCores)
    if (!config.clientMode) {
      // startNettyRpcEnv是一个偏函数
