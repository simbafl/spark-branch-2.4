 ...
 *
 * Executor节点可以使用Driver节点的RpcEnv提供的 NettyStreamManager，从Driver将Jar包拉取到Executor节点
 */
private[netty] class NettyStreamManager(rpcEnv: NettyRpcEnv)
  ...
  override def openStream(streamId: String): ManagedBuffer = {
    /** 整体逻辑：
     * 从缓存中获取文件后
     * 将 TransportConf 及 File等信息封装为 FileSegmentManagedBuffer 返回
     * FileSegmentManagedBuffer 封装了对文件的流处理细节
     */
