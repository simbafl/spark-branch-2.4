...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    // 创建 ConsoleProgressBar，UI_SHOW_CONSOLE_PROGRESS默认属性为true
    _progressBar =
    ...
    // 通过 SparkUI.create 方法创建 SparkUI
    _ui =
    ...
    // 向ListenerBus投递应用启动事件
    postApplicationStart();

    // 等待SchedulerBackend准备完成
    _taskScheduler.postStartHook();
    // 向度量系统注册 BlockManagerSource 和 DAGScheduler
  ...
  def broadcast[T: ClassTag](value: T): Broadcast[T] = {
    ...
    // 调用 SparkEnv的子组件 BroadcastManager的 newBroadcast 方法生成广播对象
   ...
   * 用于向LiveListenerBus添加实现了特质SparkListenerInterface的监听器
   */
  @DeveloperApi
  ...
  def runJob[T, U: ClassTag](
    ...
    // 将RDD及已经构建好的DAG提交给 DagScheduler 进行调度
    dagScheduler.runJob(rdd, cleanedFunc, partitions, callSite, resultHandler, localProperties.get)
    progressBar.foreach(_.finishAll())
    rdd.doCheckpoint()  // 保存 checkpoint
  }
   ...
   *
   * Spark提供自定义 SparkListener 的地方
   * 从conf的属性中获取用户自定义的SparkListener的类名，可以通过逗号分隔多个自定义SparkListener
   * EXTRA_LISTENERS
   */
  private def setupAndStartListenerBus(): Unit = {
    try {
      // 通过反射生成每一个自定义的SparkListener的实例，并添加到事件总线的监听器列表中
  ...
  private def postEnvironmentUpdate() {
    if (taskScheduler != null) {
      ...
      // 生成事件并投递到 listenerBus，此事件将被 EnvironmentListener监听
...
object SparkContext extends Logging {
  ...
  /**
   * 对SparkContext进行构造时使用的锁，以此保证构造SparkContext的过程时线程安全.
   */
   ...
   *
   * 保存激活的SparkContext，类型为 AtomicReference.
   */
   ...
   *
   * 用于将SparkContext作为 Context，保存到activeContext
   */
   ...
   * 创建 TaskScheduler，接收元祖 (SchedulerBackend, TaskScheduler).
   */
