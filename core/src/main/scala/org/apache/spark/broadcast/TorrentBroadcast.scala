...
private[spark] class TorrentBroadcast[T: ClassTag](obj: T, id: Long)
   ...
   *
   * 需要时获取，从Executor或Driver上读取的广播块的值
   */
  @transient private lazy val _value: T = readBroadcastBlock();

  /** The compression codec to use, or None if compression is disabled
   * 此处 compressionCodec 与 序列化管理器中 compressionCodec 保持一致
   * */
  @transient private var compressionCodec: Option[CompressionCodec] = _;
  /** Size of each block. Default value is 4MB.  This value is only read by the broadcaster.
   * 只读属性，可以通过 spark.broadcast.blockSize进行配置
   * */
  ...
  // 广播id，broadcastId实际是样例类BroadcastBlockId
  ...
  private def writeBlocks(value: T): Int = {
    ...
    // 从sparkEnv获取 blockManager
    val blockManager = SparkEnv.get.blockManager;
    // putSingle方法将广播对象写入本地存储体系
    if (!blockManager.putSingle(broadcastId, value, MEMORY_AND_DISK, tellMaster = false)) {
      throw new SparkException(s"Failed to store $broadcastId in BlockManager")
    }
    try {
      // 将对象转化成一系列的块，块大小由 blockSize 决定
      val blocks =
      ...
      blocks.zipWithIndex.foreach { case (block, i) =>
        ...
        // 给当前分片广播块生成分片的 BroadcastBlockId
        val pieceId = BroadcastBlockId(id, "piece" + i)
        val bytes = new ChunkedByteBuffer(block.duplicate());
        // putBytes 方法将分片广播块以序列化的方式写入Driver本地存储
   ...
   * 如果本地对象查不到，就要从driver端或executor端磁盘读取 block
   */
  private def readBlocks(): Array[BlockData] = {
    ...
    for (pid <- Random.shuffle(Seq.range(0, numBlocks))) {
      ...
      // 从 blockManager 中读取块，也就是从本地磁盘读序列化块
      bm.getLocalBytes(pieceId) match {
        case Some(block) =>
          blocks(pid) = block
          releaseLock(pieceId); // 这个锁保证当块被一个运行中的任务使用时，不能被其他任务再次使用
        // getRemoteBytes 从远程的节点读取序列化块
    ...
    TorrentBroadcast.synchronized {
      ...
      Option(broadcastCache.get(broadcastId)).map(_.asInstanceOf[T]).getOrElse {
        ...
        blockManager.getLocalValues(broadcastId) match {
          ...
          // 如果没有从本地的 blockManager 中获取到广播对象
          case None =>
