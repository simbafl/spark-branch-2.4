...
private[spark] abstract class MapOutputTracker(conf: SparkConf) extends Logging {
  /** 用于持有Driver上 MapOutputTrackerMasterEndpoint 的RpcEndpointRef. */
   ...
   *
   * epoch作为记录，每一次map output丢失加1
   */
  protected var epoch: Long = 0
  protected val epochLock = new AnyRef; // 锁，用于保证epoch变量的线程安全性

  /**
   * Send a message to the trackerEndpoint and get its result within a default timeout, or
   * throw a SparkException if this fails.
   *
   * 向MapOutputTrackerMasterEndpoint发送异步消息
   */
 ...
 *
 * 默认提到的MapOutputTracker即为MapOutputTrackerMaster，存在于driver端
 * executor端的MapOutputTrackerWorker 将map信息发送到master
 * 由MapOutputTrackerMaster负责整理和维护所有的map任务的输出跟踪信息
 * MapOutputTrackerMasterEndpoint 存在于 MapOutputTrackerMaster，MapOutputTrackerWorker持有其 Ref
 */
private[spark] class MapOutputTrackerMaster(
  ...
  // 阻塞队列来缓存GetMapOutputMessage，即map任务输出
  private val mapOutputRequests = new LinkedBlockingQueue[GetMapOutputMessage];

  // 用于获取map输出的固定大小的线程池，大小默认为8
  // 此处 线程池的创建与 Dispatcher相同，但是 MessageLoop的逻辑不同
  private val threadpool: ThreadPoolExecutor = {
    val numThreads = conf.getInt("spark.shuffle.mapOutput.dispatcher.numThreads", 8);
    // 创建线程池，并且启动的线程都以后台线程方式运行，前缀定义好
  ...
  private class MessageLoop extends Runnable {
    override def run(): Unit = {
      try {
        while (true) {
          try {
            val data = mapOutputRequests.take(); // 此处data就是 GetMapOutputMessage
             if (data == PoisonPill) {
              // 如果是毒药，重新放入队列
