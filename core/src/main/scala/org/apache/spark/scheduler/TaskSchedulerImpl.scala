 ...
 *
 * 1. 底层通过操作SchedulerBackend 针对不同种类的模式 yarn、local、mesos 调度task
 * 2. 也可以通过使用一个LocalBackend，并且将isLocal参数设置为true，来在本地模式下工作
 * 3. 它负责处理一些通用的逻辑，比如说决定多个job的调度顺序，启动推测任务执行
 * 4. 客户端首先调用initialize()方法和start()方法，然后通过runTasks()方法提交task sets
 */
